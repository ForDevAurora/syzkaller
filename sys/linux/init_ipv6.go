...
func (arch *arch) generateIpv6Addr(g *prog.Gen, typ0 prog.Type, dir prog.Dir, old prog.Arg) (
	...
	// use ipv6
	if critical_structure == nil || critical_structure_part2 == nil || len(*critical_structure) == 0 || len(*critical_structure_part2) == 0 {
		ipv6_temp_1 := g.GenerateSpecialArgAddr(typ.Fields[0].Type, prog.DirIn, &calls).(*prog.ResultArg) // ipv6_part1 resource
	...
	if new_ipv6_part1 == nil || new_ipv6_part2 == nil {
		panic("generateIpv6Addr: no existing correlation found for ipv6_addr or mac_addr")
	}
	arg = prog.MakeGroupArg(typ, dir, []prog.Arg{prog.MakeResultArg(typ.Fields[0].Type, typ.Fields[0].Dir(0), new_ipv6_part1, 0),
		prog.MakeResultArg(typ.Fields[1].Type, typ.Fields[1].Dir(0), new_ipv6_part2, 0)})

	// Originally complex logic
	...
	// arg = prog.MakeGroupArg(typ, dir, []prog.Arg{prog.MakeResultArg(typ.Fields[0].Type, typ.Fields[0].Dir(0), ipv6_part1_ref, 0),
	// 	prog.MakeResultArg(typ.Fields[1].Type, typ.Fields[1].Dir(0), ipv6_part2_ref, 0)})
	return
}
